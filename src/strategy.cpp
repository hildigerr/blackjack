...
// For each (non-bust, non-blackjack) possible outcome, accumulate probability
// of each hand with that count (that is actually possible in the given shoe).
...
// If it is possible (i.e., a subset of the shoe), compute probability of the
// hand.  Note the initial value of p; we save 10 multiplications in the next
// step by doing part of the "conditioning" here.
...
// For each up card, a certain number of permutations of the cards in the hand
// are possible, each equally likely.  Count these, conditioned on each up
// card.
...
// To only count each hand (subset) once, only draw cards higher than any in
// the hand.
...
// If the hand is not busted (or could be a split hand), record it and
// partially link it up; we'll finish with linkHands() later.
...
// This would be easier if not for the fact that hitting split aces is not
// allowed.
...
// This particular traversal of hands isn't really necessary for standing and
// doubling down; it is simply useful to use the already-identified list of
// valid hands (depending on whether we are splitting, how many hands, etc.).
...
// By computing values for E(hit) in the proper order, we guarantee that any
// values needed for computing the value of a given hand will be available.
// We start with the hard hands, but stop at hard 11, since we could draw an
// ace to a 10, making a soft 21.
